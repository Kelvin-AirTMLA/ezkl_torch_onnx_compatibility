"""
TrustAgent — EZKL settings generation and scale calibration.

Maps to spec routines: gen_settings() + calibrate() → calibrate_settings() in Python bindings.
"""
...
class CreditScorerMLP(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.fc1 = nn.Linear(INPUT_DIM, 16, dtype=torch.float32)
        self.fc2 = nn.Linear(16, 16, dtype=torch.float32)
        self.fc3 = nn.Linear(16, OUTPUT_DIM, dtype=torch.float32)
        self.relu = nn.ReLU()
...
def refresh_input_json() -> None:
    """Write input.json with flat layout required by ezkl (no nested arrays)."""
    rng = np.random.default_rng(0)
    sample = rng.uniform(0.0, 1.0, size=(1, INPUT_DIM)).astype(np.float32)
    sample[0, 2] = float(rng.integers(0, 2))
